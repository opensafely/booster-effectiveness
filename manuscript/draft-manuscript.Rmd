...
Additionally, if the rolling weekly average count of BNT162b2 booster doses within strata defined by region, JCVI priority group, and the week of second dose was below 50 then recruitment did not occur for that stratum on that day. This helped to ensure that recruitment was restricted to those eligible and able to receive a booster dose in line with national prioritisation schedules at the time of potential recruitment.
...
We also estimated booster effectiveness separately in the following subgroups: primary vaccine course; age (18-64 or $\ge$ 65 years); clinically extremely vulnerable or not; evidence of prior SARS-CoV-2 infection or not.
...
There were `r label_number(1, big.mark=",")(filter(incidence, treatment=="pfizer", outcome=="postest", fup_period=="All") %>% pull(events))` positive SARS-CoV-2 tests,
`r label_number(1, big.mark=",")(filter(incidence, treatment=="pfizer", outcome=="covidadmitted", fup_period=="All") %>% pull(events))` COVID-19 hospital admissions, 
`r label_number(1, big.mark=",")(filter(incidence, treatment=="pfizer", outcome=="coviddeath", fup_period=="All") %>% pull(events))` COVID-19 deaths, 
and `r label_number(1, big.mark=",")(filter(incidence, treatment=="pfizer", outcome=="noncoviddeath", fup_period=="All") %>% pull(events))` non-COVID-19 deaths 
across `r label_number(1, big.mark=",")((filter(incidence, treatment=="pfizer", outcome=="coviddeath", fup_period=="All") %>% pull(yearsatrisk)) * 365.25/7)` person-weeks of follow-up. 
...
respectively (`r km70 %>% filter(treatment=="pfizer", outcome=="covidadmitted", subgroup=="none") %>% pull(kmrd) %>% (label_number(0.1, scale=-1000))` fewer events in the boosted group). 
...
The rate of increase in the cumulative incidence of positive SARS-CoV-2 tests in boosted people tapered substantially around 7 days after boosting, something not observed in unboosted controls (Figure 2). Similar patterns were not observed for more severe outcomes, with differences in the cumulative incidence between the groups apparent in the first few days. 
...
Amongst people classified as clinically extremely vulnerable, estimated booster effectiveness was `r filter(overallhazards, outcome=="postest", treatment=="pfizer", subgroup=="cev-TRUE", model==3) %>% pull(ve_display)` for positive SARS-CoV-2 test, `r filter(overallhazards, outcome=="covidadmitted", treatment=="pfizer", subgroup=="cev-TRUE", model==3) %>% pull(ve_display)` for COVID-19 hospital admission, `r filter(overallhazards, outcome=="coviddeath", treatment=="pfizer", subgroup=="cev-TRUE", model==3) %>% pull(ve_display)` for COVID-19 death, and `r filter(overallhazards, outcome=="noncoviddeath", treatment=="pfizer", subgroup=="cev-TRUE", model==3) %>% pull(ve_display)` for non-COVID-19 death. For people who were not classified as clinically extremely vulnerable, estimated booster effectiveness was similar for positive SARS-CoV-2 tests (`r filter(overallhazards, outcome=="postest", treatment=="pfizer", subgroup=="cev-FALSE", model==3) %>% pull(ve_display)` and non-COVID-19 death (`r filter(overallhazards, outcome=="noncoviddeath", treatment=="pfizer", subgroup=="cev-FALSE", model==3) %>% pull(ve_display)`), but higher for COVID-19 hospital admission and death (`r filter(overallhazards, outcome=="covidadmitted", treatment=="pfizer", subgroup=="cev-FALSE", model==3) %>% pull(ve_display)` and `r filter(overallhazards, outcome=="coviddeath", treatment=="pfizer", subgroup=="cev-FALSE", model==3) %>% pull(ve_display)` respectively). 

Amongst people aged $\ge$ 65 years, estimated booster effectiveness was `r filter(overallhazards, outcome=="postest", treatment=="pfizer", subgroup=="age65plus-TRUE", model==3) %>% pull(ve_display)` for positive SARS-CoV-2 test, `r filter(overallhazards, outcome=="covidadmitted", treatment=="pfizer", subgroup=="age65plus-TRUE", model==3) %>% pull(ve_display)` for COVID-19 hospital admission, `r filter(overallhazards, outcome=="coviddeath", treatment=="pfizer", subgroup=="age65plus-TRUE", model==3) %>% pull(ve_display)` for COVID-19 death, and `r filter(overallhazards, outcome=="noncoviddeath", treatment=="pfizer", subgroup=="age65plus-TRUE", model==3) %>% pull(ve_display)` for non-COVID-19 death.  Corresponding effectiveness was similar in people aged 18-64 years for positive SARS-CoV-2 tests (`r filter(overallhazards, outcome=="postest", treatment=="pfizer", subgroup=="age65plus-FALSE", model==3) %>% pull(ve_display)`), but slightly lower for more severe outcomes: `r filter(overallhazards, outcome=="covidadmitted", treatment=="pfizer", subgroup=="age65plus-FALSE", model==3) %>% pull(ve_display)` for COVID-19 hospital admission, `r filter(overallhazards, outcome=="coviddeath", treatment=="pfizer", subgroup=="age65plus-FALSE", model==3) %>% pull(ve_display)` for COVID-19 death, and `r filter(overallhazards, outcome=="noncoviddeath", treatment=="pfizer", subgroup=="age65plus-FALSE", model==3) %>% pull(ve_display)` for non-COVID-19 death.

Outcomes were similar for COVID-19-related events when comparing people with or without evidence of prior SARS-CoV-2 infection. Estimated booster effectiveness for those with evidence of prior infection was `r filter(overallhazards, outcome=="postest", treatment=="pfizer", subgroup=="prior_covid_infection-TRUE", model==3) %>% pull(ve_display)` for positive SARS-CoV-2 test, `r filter(overallhazards, outcome=="covidadmitted", treatment=="pfizer", subgroup=="prior_covid_infection-TRUE", model==3) %>% pull(ve_display)` for COVID-19 hospital admission, `r filter(overallhazards, outcome=="coviddeath", treatment=="pfizer", subgroup=="prior_covid_infection-TRUE", model==3) %>% pull(ve_display)` for COVID-19 death. Corresponding estimates in people with no evidence of prior infection were `r filter(overallhazards, outcome=="postest", treatment=="pfizer", subgroup=="prior_covid_infection-FALSE", model==3) %>% pull(ve_display)`, `r filter(overallhazards, outcome=="covidadmitted", treatment=="pfizer", subgroup=="prior_covid_infection-FALSE", model==3) %>% pull(ve_display)`, and `r filter(overallhazards, outcome=="coviddeath", treatment=="pfizer", subgroup=="prior_covid_infection-FALSE", model==3) %>% pull(ve_display)`, respectively. For non-COVID-19, estimated booster effectiveness was slightly lower in those with prior evidence of SARS-CoV-2 infection (`r filter(overallhazards, outcome=="noncoviddeath", treatment=="pfizer", subgroup=="prior_covid_infection-TRUE", model==3) %>% pull(ve_display)`) than in those without (`r filter(overallhazards, outcome=="noncoviddeath", treatment=="pfizer", subgroup=="prior_covid_infection-FALSE", model==3) %>% pull(ve_display)`).

Booster effectiveness was also estimated using narrower periods, with effectiveness against positive SARS-CoV-2 tests appearing to wane in later periods (`r filter(hazards, outcome=="postest", treatment=="pfizer", term_right==28, subgroup=="none", model==3) %>% pull(ve_display)` during days 15-28 after boosting versus `r filter(hazards, outcome=="postest", treatment=="pfizer", term_right==70, subgroup=="none", model==3) %>% pull(ve_display)` during days 43-70), a pattern that persisted in primary course subgroup analyses (Figure 3). For more severe outcomes, period-specific effectiveness estimates are less clear due to fewer events, though were broadly similar between different primary course brands.
