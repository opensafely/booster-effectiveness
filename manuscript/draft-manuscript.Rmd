...
incidences %>%
  ...
  left_join(
    matchsummary %>% select(treatment, eligible_1=n_eligible, recruit_prop_1 = prop_matched_eligible),
    by=c("treatment")
  ) %>%
  select(
    treatment_descr, 
...
For the same period, the estimated effectiveness for BNT162b2 and ChAdOx1-S primary courses respectively were `r filter(hazards_vax12_type, outcome=="postest", treatment=="pfizer", subgroup_level=="pfizer-pfizer", model==3, term_left==28) %>% pull(hr_display)` and `r filter(hazards_vax12_type, outcome=="postest", treatment=="pfizer", subgroup_level=="az-az", model==3, term_left==28) %>% pull(hr_display)` for a positive test, `r filter(hazards_vax12_type, outcome=="covidadmitted", treatment=="pfizer", subgroup_level=="pfizer-pfizer", model==3, term_left==28) %>% pull(hr_display)` and `r filter(hazards_vax12_type, outcome=="covidadmitted", treatment=="pfizer", subgroup_level=="az-az", model==3, term_left==28) %>% pull(hr_display)` for COVID-19 hospital admission, and `r filter(hazards_vax12_type, outcome=="coviddeath", treatment=="pfizer", subgroup_level=="pfizer-pfizer", model==3, term_left==28) %>% pull(hr_display)` and `r filter(hazards_vax12_type, outcome=="coviddeath", treatment=="pfizer", subgroup_level=="az-az", model==3, term_left==28) %>% pull(hr_display)` for COVID-19 death (Figure 2).
...
This observational study of fully vaccinated adults in England estimated high protection of BNT162b2 booster vaccination against all studied outcomes. 
Protection appeared higher in those receiving a X-X primary course.
There were apparent waning effects after X weeks which is unlikely to explained by increased transmissibility of the Omicron variant given Omicron was not yet dominant in the UK by the end of the follow-up window (ref). 

### Limitations

We used routinely-collected health records with comprehensive coverage of primary care, hospital admissions, COVID-19 testing and vaccination, and death registrations to study over X million fully vaccinated English residents ...
...
[pfizer phase 3 booster trial, in pfizer first course], [ cov-boost trial]

A phase III trial assessing BNT162b2 booster efficacy in two-dose BNT162b2 recipients found.. (ref)
