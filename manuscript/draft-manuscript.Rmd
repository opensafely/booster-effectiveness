...
hazards <- read_csv(fs::path(output_dir_os, "effects.csv")) %>%
  mutate(
    treatment_descr = fct_inorder(treatment_descr),
    outcome_descr = fct_inorder(outcome_descr),
    #model_name = fct_inorder(str_wrap(model_name, 15)),
...
William J Hulme^1^, Elizabeth J Williamson^2^, Amelia Green^1^, Elsie Horne^4,5^, Linda Nab^1^, Ruth Keogh^2^, Edward Parker^2^, Jonathan AC Sterne^4,5^, Miguel Hernán^6,7^, Ben Goldacre^1^ + OpenSAFELY collaborative.
...
We studied adults (18 and over) in England who had received a complete primary COVID-19 vaccination course (two doses) of BNT162b2 or ChAdOx1-S and who were registered at a GP practice using TPP's SystmOne clinical information system. People were not eligible for inclusion if: they were a vaccinated health care worker, resident in a care or nursing home, medically housebound or receiving end-of-life care (due to distinct clustering of vaccine uptake and infection risk in these groups that could not be reasonably measured and controlled for); had received heterologous (i.e., mixed) first and second vaccine dose; information on sex, ethnicity, deprivation, and geographical region was not known.

After exclusions for missing values on demographic variables, there were no missing values in remaining variables as they were defined by the presence or absence of clinical codes or events in the patient record.

We used a sequential trials approach designed to emulate a sequence of two-arm parallel randomised controlled trials recruiting participants between `r format(as.Date(study_dates$studystart_date), "%d %B %Y")` to `r format(as.Date(study_dates$studyend_date), "%d %B %Y")` inclusive. For each trial, the treatment arm comprises people who received a booster dose on that day, and the control arm comprises the same number of unboosted people selected using exact 1:1 matching without replacement. Trial arms are matched on primary course vaccine type (BNT162b2, ChAdOx1-S, or mRNA-1273), the calendar week of the second vaccine dose, geographical region, prior infection status, and vaccine priority group (as determined by the Joint Committee for Vaccine and immunisation expert working group). People selected as controls are not eligible to be included again as a control in a subsequent trial, but are eligible for selection into the treatment arm of a subsequent trial. Unmatched booster recipients were not included.

Additional eligiblity criteria based on sufficiently many people being vaccinated on each day within region, priority group, and second dose week strata were also used to ensure unboosted people were both eligible and able to receive a booster dose at the time of potential recruitment. The trial sequence was terminated early if fewer than 10% of treated people were successfully matched. See supplementary materials for further details.


## Outcomes

The following five outcomes were considered: positive SARS-CoV-2 test; COVID-19 A&E attendance; unplanned COVID-19 hospital admission; COVID-19 critical care admission; COVID-19 death. Positive SARS-CoV-2 tests were identified using SGSS records and based on swab date. Both polymerase chain reaction (PCR) and lateral flow tests are included, without differentiation between symptomatic and asymptomatic infection. COVID-19 A&E attendances were identified using HES emergency care records with U07.1 ("COVID-19, virus identified") or U07.2 ("COVID-19, virus not identified") ICD-10 diagnosis codes @emergenc. Unplanned COVID-19 hospital admissions and critical care admissions were identified using HES in-patient hospital records with U07.1 or U07.2 reason for admission ICD-10 codes. COVID-19 deaths with ICD-10 codes (as above) mentioned anywhere on the death certificate (i.e., as an underlying or contributing cause of death) were included.
...
A total of `r label_number(1, big.mark=",")(filter(flowchart, crit=="c0") %>% pull(n))` fully vaccinated (received a primary course of two COVID-19 vaccine doses) people aged 18 and over and registered at a TPP practice on `r format(as.Date(study_dates$studystart_date), "%d %B %Y")` were identified the database, with `r label_number(1, big.mark=",")(filter(flowchart, crit=="c4") %>% pull(n))` (`r label_percent(0.1)(filter(flowchart, crit=="c4") %>% pull(pct_all))`) eligible for inclusion (supplementary Tables S1 and S2). Of these, `r label_number(1, big.mark=",")(cohortsummary$n_boosted)` received a booster dose during the recruitment period (up to `r format(as.Date(study_dates$lastvax3_date), "%d %B %Y")`). `r label_number(1, big.mark=",")(cohortsummary$n_3pfizer)` ( `r label_percent(0.1)(cohortsummary$prop_3pfizer)`) for BNT162b2 and `r label_number(1, big.mark=",")(cohortsummary$n_3moderna)` ( `r label_percent(0.1)(cohortsummary$prop_3moderna)`) for mRNA-1273 (Figure 1).

## Matching

`r label_number(1, big.mark=",")(filter(matchsummary, treatment=="pfizer") %>% pull(n))` (`r label_percent(0.1)(filter(matchsummary, treatment=="pfizer") %>% pull(prop_recruited))`) BNT162b2 booster recipients were successfully matched to unboosted controls, contributing `r label_number(1, big.mark=",")(filter(matchsummary, treatment=="pfizer") %>% pull(fup_years))` person-years of follow-up across both trial arms. For mRNA-1273, this was `r label_number(1, big.mark=",")(filter(matchsummary, treatment=="moderna") %>% pull(n))` (`r label_percent(0.1)(filter(matchsummary, treatment=="moderna") %>% pull(prop_recruited))`) and `r label_number(1, big.mark=",")(filter(matchsummary, treatment=="moderna") %>% pull(fup_years))` person-years of follow-up (Table 2). 

Characteristics at recruitment between treatment and controls arms are provided in supplementary materials (Table S2). Characteristics were (??) well-balanced between trial arms.


\newpage

#### Figure 1: Booster vaccination uptake, and matching success

Booster vaccination update over the duration of the study cohort, and the proportion of those that were matched for modelling.
...
matchcoverage  %>%
  ...
  geom_col(
    aes(
      x=vax3_date+0.5,
      y=cumuln,
      group=fct_rev(matched),
      fill=fct_rev(matched),
      colour=NULL
    ),
...
Participant characteristics as on the day of recruitment into the treatment or control arms, stratified by treatment and outcome type. 
...
In the BNT162b2 and mRNA-1273 analyses respectively, matched participants contributed `r filter(incidences, treatment=="pfizer", outcome=="postest", fup_period=="all") %>% pull(events)` and `r filter(incidences, treatment=="moderna", outcome=="postest", fup_period=="all") %>% pull(events)` positive SARS-CoV-2 tests, `r filter(incidences, treatment=="pfizer", outcome=="covidattendance", fup_period=="all") %>% pull(events)` and `r filter(incidences, treatment=="moderna", outcome=="covidattendance", fup_period=="all") %>% pull(events)` COVID-19 A&E attendances, `r filter(incidences, treatment=="pfizer", outcome=="covidadmitted", fup_period=="all") %>% pull(events)` and `r filter(incidences, treatment=="moderna", outcome=="covidadmitted", fup_period=="all") %>% pull(events)` COVID-19 hospital admissions, `r filter(incidences, treatment=="pfizer", outcome=="covidcc", fup_period=="all") %>% pull(events)` and `r filter(incidences, treatment=="moderna", outcome=="covidcc", fup_period=="all") %>% pull(events)` COVID-19 ciritical care admissions, and `r filter(incidences, treatment=="pfizer", outcome=="coviddeath", fup_period=="all") %>% pull(events)` and `r filter(incidences, treatment=="moderna", outcome=="coviddeath", fup_period=="all") %>% pull(events)` COVID-19 deaths (Table 2). 

During the fifth and sixth week after recruitment (days 28-41) the estimated hazard ratio comparing a BNT162b2 booster dose to no dose for a positive test was `r filter(hazards, outcome=="postest", treatment=="pfizer", model==3, term_left==28) %>% pull(hr_display)`, for COVID-19 A&E attendances was `r filter(hazards, outcome=="covidattendance", treatment=="pfizer", model==3, term_left==28) %>% pull(hr_display)`, for COVID-19 hospital admission was `r filter(hazards, outcome=="covidadmitted", treatment=="pfizer", model==3, term_left==28) %>% pull(hr_display)`, for COVID-19 critical care admission was `r filter(hazards, outcome=="covidcc", treatment=="pfizer", model==3, term_left==28) %>% pull(hr_display)`, and for COVID-19 death was `r filter(hazards, outcome=="coviddeath", treatment=="pfizer", model==3, term_left==28) %>% pull(hr_display)` (Figure 2). For mRNA-1273, the corresponding estimated hazard ratios were `r filter(hazards, outcome=="postest", treatment=="moderna", model==3, term_left==28) %>% pull(hr_display)`, for COVID-19 A&E attendances was `r filter(hazards, outcome=="covidattendance", treatment=="moderna", model==3, term_left==28) %>% pull(hr_display)`, for COVID-19 hospital admission was `r filter(hazards, outcome=="covidadmitted", treatment=="moderna", term_left==28) %>% pull(hr_display)`, for COVID-19 critical care admission was `r filter(hazards, outcome=="covidcc", treatment=="moderna", model==3, term_left==28) %>% pull(hr_display)`, and for COVID-19 death was `r filter(hazards, outcome=="coviddeath", treatment=="moderna", model==3, term_left==28) %>% pull(hr_display)`. 


Estimated effectiveness by primary vaccine course...
...
For each outcome based on the fully adjusted model, the period-specific hazard ratio for boosting versus not boosting is shown. Models with less extensive confounder adjustment are provided in supplementary materials (supplementary Figure SX). 
