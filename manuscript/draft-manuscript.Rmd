...
incidence_all <- read_csv(fs::path(output_dir_os, "none", "incidence_all.csv")) %>% mutate(events=events_1+events_0)
...
groupvariables <- c("treatment", "treatment_descr", "outcome", "outcome_descr", "outcome_descr_wrap", "subgroup", "subgroup_descr", "subgroup_level", "subgroup_level_descr")
...
kmconstrast <- function(data, cuts=NULL){
  ...
  data %>%
    ...
    mutate(
      cml.events = cml.event_0 + cml.events_1,
...
We estimated Kaplan-Meier cumulative incidence curves separately in the boosted and unboosted groups, and the corresponding 10-week risk difference and risk ratio. 
...
and `r label_number(1, big.mark=",")(filter(incidences, treatment=="pfizer", outcome=="noncovidadmitted", fup_period=="All") %>% pull(events))` non-COVID-19 deaths 
across `r label_number(1, big.mark=",")((filter(incidences, treatment=="pfizer", outcome=="coviddeath", fup_period=="All") %>% pull(yearsatrisk)) * 365.25/7)` person-weeks of follow-up. 
...
Amongst people with evidence of prior SARS-CoV-2 infection, estimated booster effectiveness was `r filter(overallhazards, outcome=="postest", treatment=="pfizer", subgroup=="prior_covid_infection-TRUE", model==3) %>% pull(ve_display)` for positive SARS-CoV-2 test, `r filter(overallhazards, outcome=="covidadmitted", treatment=="pfizer", subgroup=="prior_covid_infection-TRUE", model==3) %>% pull(ve_display)` for COVID-19 hospital admission, `r filter(overallhazards, outcome=="coviddeath", treatment=="pfizer", subgroup=="prior_covid_infection-TRUE", model==3) %>% pull(ve_display)` for COVID-19 death, and `r filter(overallhazards, outcome=="noncoviddeath", treatment=="pfizer", subgroup=="prior_covid_infection-TRUE", model==3) %>% pull(ve_display)` for non-COVID-19 death.  Corresponding effectiveness was ... in people with no evidence of prior infection: `r filter(overallhazards, outcome=="postest", treatment=="pfizer", subgroup=="prior_covid_infection-FALSE", model==3) %>% pull(ve_display)`, `r filter(overallhazards, outcome=="covidadmitted", treatment=="pfizer", subgroup=="prior_covid_infection-FALSE", model==3) %>% pull(ve_display)`, `r filter(overallhazards, outcome=="coviddeath", treatment=="pfizer", subgroup=="prior_covid_infection-FALSE", model==3) %>% pull(ve_display)`, and `r filter(overallhazards, outcome=="noncoviddeath", treatment=="pfizer", subgroup=="prior_covid_infection-FALSE", model==3) %>% pull(ve_display)` respectively.
...
incidence_table <- 
  incidence %>%
  left_join(
    kmdiff %>% filter(time == postbaselinecuts[-1] -1),
    by = c("treatment", "outcome", "subgroup", )
  ) %>%
  filter(
    #fup_period == "All",
    outcome %in% outcomes,
