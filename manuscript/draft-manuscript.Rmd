...
kmdiff <- km %>% 
  ...
  select(
    ...
    treated, surv, time, leadtime
  ) %>%
  pivot_wider(
    id_cols= c("treatment", "outcome", "subgroup", "time", "leadtime"),
    names_from=treated,
    names_prefix="surv_",
    values_from=surv
  ) %>%
...
km70 <- kmdiff %>% filter(leadtime==max(postbaselinecuts))
# 
...
incidences %>%
  left_join(kmdiff %>% filter(time = postbaselinecuts[-1] -1)) %>%
  filter(
    fup_period == "All",
    outcome %in% outcomes,
    treatment == "pfizer"
