...
formatpercent100 <- function(x,accuracy){
 ...
 ## hazard ratio
  ...
  plot_hazards <-
    ...
    geom_point(aes(y=hr, x=term_midpoint, colour=treatment_descr), position = position_dodge(width = 1.8))+
    geom_linerange(aes(ymin=hr.ll, ymax=hr.ul, x=term_midpoint, colour=treatment_descr), position = position_dodge(width = 1.8))+
    geom_point(data = overallhazards_plotdata, mapping = aes(y=hr, x=max(postbaselinecuts28+14), colour=treatment_descr), position = position_dodge(width = 1.8)) +
    geom_linerange(data = overallhazards_plotdata, mapping = aes(ymin=hr.ll, ymax=hr.ul, x=max(postbaselinecuts28+14), colour=treatment_descr), position = position_dodge(width = 1.8)) +
    geom_rect(data = overallhazards_plotdata, ymin=-Inf, ymax=Inf, xmin=max(postbaselinecuts28+14)-7, xmax=max(postbaselinecuts28+14)+7, fill="grey95", alpha=0.4, colour="transparent") +
    geom_hline(aes(yintercept=1), colour='grey')+
    facet_grid(rows=vars(subgroup_level_descr_wrap), cols=vars(outcome_descr_wrap), switch="y")+
    scale_y_log10(
      breaks=y_breaks,
    ...
    scale_x_continuous(breaks=c(postbaselinecuts28, max(postbaselinecuts28+14)), labels=c(postbaselinecuts28, ""), limits=c(min(postbaselinecuts28), max(postbaselinecuts28+14)+14), expand = c(0, 0))+
